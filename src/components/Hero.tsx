...
export function Hero() {
  ...
  return (
    <section id="top" className="relative z-10 px-3 pt-24 pb-10 md:px-6 md:pt-28 md:pb-16">
      <div className="glass-stage relative mx-auto min-h-[calc(100svh-7rem)] max-w-7xl overflow-hidden rounded-[2rem] border border-white/12 md:rounded-[2.5rem]">
        {/* inner glow frame */}
        <div className="pointer-events-none absolute inset-0 rounded-[inherit] shadow-[inset_0_0_0_1px_rgba(255,255,255,0.06),inset_0_1px_0_rgba(255,255,255,0.12)]" />
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_60%_40%,rgba(215,255,50,0.14),transparent_55%)]" />

        {/* floating lime orb */}
        <motion.div
          aria-hidden
          className="pointer-events-none absolute top-[18%] left-1/2 size-[min(55vw,420px)] -translate-x-1/2 rounded-full"
          style={{
            background:
              "radial-gradient(circle at 35% 35%, rgba(215,255,50,0.85), rgba(215,255,50,0.15) 45%, transparent 70%)",
            filter: "blur(2px)",
          }}
          animate={
            reduce
        ...
        <motion.div
          aria-hidden
          className="pointer-events-none absolute top-[28%] left-[42%] size-[min(28vw,200px)] rounded-full bg-white/10 blur-2xl"
          animate={reduce ? undefined : { x: [0, 30, 0], y: [0, 20, 0] }}
          transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
        />

        <div className="relative grid min-h-[calc(100svh-7rem)] items-center gap-8 p-6 md:grid-cols-[1.05fr_0.95fr] md:gap-6 md:p-10 lg:p-14">
          ...
          {/* product stage — phone + floating cards */}
        ...
        <p className="absolute right-6 bottom-5 hidden text-[10px] tracking-[0.18em] text-white/25 uppercase md:block">
          * przykładowy KPI UI — nie case klienta
        </p>
      </div>
    </section>
  );
